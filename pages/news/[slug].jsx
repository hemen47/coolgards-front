...
export default function Post({ data, error, relatedPosts }) {
  ...
  const formattedModifiedDate = data?.data?.updatedAt
    ? format(new Date(data.data.updatedAt), 'yyyy-MM-dd')
  ...
  return (
    <>
      <Head>
        <title>
          {data?.data.title
            ? `${data.data.title} | CoolGards Cold Compression Therapy`
            : 'CoolGards News'}
        </title>
        <meta
          name="description"
          content={
            data?.data.excerpt ||
            `Learn about ${data?.data.title} and how cold compression therapy can help with recovery and pain management.`
          }
        />
        <meta
          name="keywords"
          content={`${data?.data.tags?.join(
            ', '
          )}, cold compression therapy, recovery, inflammation reduction`}
        />
            ...
            <div className={styles.titleContainer}>
              ...
              <div className={styles.metaInfo}>
                ...
                {data?.data.updatedAt && data?.data.updatedAt !== data?.data.createdAt && (
                  <time
                    dateTime={formattedModifiedDate}
