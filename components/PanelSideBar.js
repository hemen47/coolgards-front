...
export default function PanelSideBar() {
  ...
  return (
    <nav
      style={{ background: 'linear-gradient(135deg, #2d3436 0%, #3d3d3d 100%)' }}
      className={`flex flex-col fixed h-screen text-white overflow-x-hidden overflow-y-auto shadow-lg transition-all duration-300 ease-in-out z-50 
    ...
    >
      <div className="flex items-center p-1 border-b border-white/10">
        <div className="flex items-center max-w-[200px]">
          <Avatar
            className={`bg-white/20 !text-white !font-medium h-8 w-8 ${expanded ? 'mr-3' : 'mr-0'}`}
          >
            {getInitials(user?.fullName)}
          </Avatar>
          <div
            className={`overflow-hidden transition-opacity duration-300 text-white ${
          ...
          >
            ...
            <span className="text-xs text-white/70 block">{user?.roles}</span>
          </div>
        </div>
        <button
...
function NavItem({ href, icon, label, active, expanded, selectionColor }) {
  return (
    <Tooltip title={!expanded ? label : ''} placement="right" arrow>
      ...
      >
        <span
          className={`min-w-6 h-6 flex items-center justify-center ${expanded ? 'mr-4' : 'mr-0'}`}
        >
          {icon}
        </span>
        {expanded && (
          <span className="whitespace-nowrap text-sm transition-opacity duration-300">{label}</span>
        )}
      </Link>
    </Tooltip>
  );
